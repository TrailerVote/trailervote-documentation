...
If correctly created, the result is a `201 Created` with a `Location` to the created product.
...
If a product already exists, it can be updatd using a `PUT` request on the `self` link.
...
| JavaScript | `trailervote-api/product/updae` |
